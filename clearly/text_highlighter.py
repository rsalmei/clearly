...
def _typed_text(p, wrap=True, kdict=False):
    if p is None:
        return NONE
    ...
    if isinstance(p, CallDescriptor):
        fargs = _typed_text(p.args, wrap=False) if p.args else ''
        if p.kwargs:
            if p.args:
                fargs += SEPARATOR
            fargs += _typed_text(p.kwargs, wrap=False, kdict=True)
    ...
    if isinstance(p, (list, tuple, set)):
        if wrap:
            ...
            f = '{}'
        return f.format(SEPARATOR.join(_typed_text(x) for x in p))
